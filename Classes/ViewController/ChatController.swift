...
class ChatController: UITableViewController, UITextViewDelegate, SendControllerDelegate {
    var buddy: Buddy
    var repository: Repository
    var responses: [String] = []
    var receivedData: NSMutableData!
    var startData: NSMutableData!
    var start2Data: NSMutableData!
    var receivedConnection: NSURLConnection!
    var startConnection: NSURLConnection!
    var start2Connection: NSURLConnection!
    var botId: String?
    var botcust2: String?

    init(buddy:Buddy, repository:Repository) {
        self.buddy = buddy
    ...
    override func loadView() {
        ...
        super.loadView()
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .add, target: self, action: #selector(ChatController.add(_:)))
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let section: Int = (self.buddy.messages).count
        if section > 0 {
            self.tableView.scrollToRow(at: IndexPath(row: 0, section: section - 1), at: .bottom, animated: false)
        }
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let message: Message = (self.buddy.messages)[(indexPath as NSIndexPath).section] 
        let cell: TextViewCell = TextViewCell.cellForTableView(tableView)
        cell.textView.text = message.text
        cell.backgroundColor = message.fromMe ? UIColor.white : UIColor(red: 0.95, green: 0.95, blue: 1, alpha: 1)
        var frame: CGRect = cell.textView.frame
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView {
        ...
        label.frame = CGRect(x: 18, y: 0, width: 290, height: 20)
        label.font = UIFont.boldSystemFont(ofSize: UIFont.smallSystemFontSize)
        label.backgroundColor = UIColor.clear
        label.textColor = UIColor.darkGray
        // inset effect
        label.shadowColor = UIColor(red: 1, green: 1, blue: 1, alpha: 0.55)
    ...
    func didSendMessage(_ text: String) {
        self.addMessage(text, fromMe: true)
    }
    ...
    func connection(_ connection: NSURLConnection, didReceiveResponse response: URLResponse) {
        if connection == startConnection {
            startData.length = 0
        } else if connection == start2Connection {
            start2Data.length = 0
        } else {
    ...
    func connectionDidFinishLoading(_ connection: NSURLConnection) {
        if connection == startConnection {
            ...
            let content = NSString(bytes: startData.bytes, length: startData.length, encoding: String.Encoding.utf8.rawValue)!
            let error: NSError? = nil
            let regex: NSRegularExpression = try! NSRegularExpression(pattern: "botid=(\\w+)", options: [])
            var matches: [AnyObject] = regex.matches(in: content as String, options: [], range: NSMakeRange(0, content.length))
            self.botId = content.substring(with: matches[0].rangeAt(1))
            NSLog("Botid is now \(botId!)")
            self.startStep2Bot()
        } else if connection == start2Connection {
                NSLog("Succeeded! Received %d bytes of data", start2Data.length)
                let content = NSString(bytes: start2Data.bytes, length: start2Data.length, encoding: String.Encoding.utf8.rawValue)!
                let regex: NSRegularExpression = try! NSRegularExpression(pattern: "name=\"botcust2\" value=\"(\\w+)", options: [])
                var matches: [AnyObject] = regex.matches(in: content as String, options: [], range: NSMakeRange(0, content.length))
                self.botcust2 = content.substring(with: matches[0].rangeAt(1))
                NSLog("botcust2 is now \(botcust2!)")
        } else {
            NSLog("Succeeded! Received %d bytes of data", receivedData.length)
            let content = NSString(bytes: receivedData.bytes, length: receivedData.length, encoding: String.Encoding.utf8.rawValue)!
            NSLog("Received \(content)")
                  
            let regex: NSRegularExpression = try! NSRegularExpression(pattern: "<b>A.L.I.C.E.:<\\/b> *(.*?)<br\\/>", options: [])
            var matches: [AnyObject] = regex.matches(in: content as String, options: [], range: NSMakeRange(0, content.length))
            let reply: String = content.substring(with: matches[0].rangeAt(1))
            NSLog("Reply: %@", reply)
    ...
    func startTheBot() {
        let url: String = "http://alice.pandorabots.com"
        NSLog("Getting url %@", url)
        let theRequest: URLRequest = URLRequest(url: URL(string: url)!, cachePolicy: .useProtocolCachePolicy, timeoutInterval: 60.0)
        startConnection = NSURLConnection(request: theRequest, delegate: self)!
        startData = NSMutableData()
    }
    
    func startStep2Bot() {
        let url: String = "http://sheepridge.pandorabots.com/pandora/talk?botid=\(self.botId!)&skin=custom_input"
        NSLog("Getting url %@", url)
        let theRequest: URLRequest = URLRequest(url: URL(string: url)!, cachePolicy: .useProtocolCachePolicy, timeoutInterval: 60.0)
        start2Connection = NSURLConnection(request: theRequest, delegate: self)!
        start2Data = NSMutableData()
    }
    
    func requestBotResponse() {
        if self.botId != nil && self.botcust2 != nil {
            let lastMessage: String = self.buddy.messages.last!.text
            let postString: String = "input=\(lastMessage.addingPercentEscapes(using: String.Encoding.utf8)!)&botcust2=\(self.botcust2!)"
            let url: String = "http://sheepridge.pandorabots.com/pandora/talk?botid=\(self.botId!)&skin=custom_input"
            let request: NSMutableURLRequest = NSMutableURLRequest(url: URL(string: url)!, cachePolicy: .useProtocolCachePolicy, timeoutInterval: 60.0)
            request.httpMethod = "POST"
            NSLog("Posting url %@\n%@", url, postString)
            request.httpBody = postString.data(using: String.Encoding.utf8)
            receivedConnection = NSURLConnection(request: request as URLRequest, delegate: self)!
            receivedData = NSMutableData()
        }
        else {
            let index: Int = abs(arc4random(Int.self) % self.responses.count)
            self.perform(#selector(ChatController.responseReceived(_:)), with: self.responses[index], afterDelay: Double(arc4random(Int.self) % 15 + 2))
        }
    }
    ...
    func addMessage(_ text: String, fromMe: Bool) {
        let msg: Message = self.repository.messageForBuddy(self.buddy)
        msg.text = text
        msg.fromMe = fromMe
        if fromMe {
            self.requestBotResponse()
        ...
        self.tableView.scrollToRow(at: IndexPath(row: 0, section: (self.buddy.messages).count - 1), at: .bottom, animated: true)
    }
    
    func add(_ sender: AnyObject) {
        let ctrl: SendController = SendController(delegate:self)
        self.present(UINavigationController(rootViewController: ctrl), animated: true, completion: nil)
    }
}
